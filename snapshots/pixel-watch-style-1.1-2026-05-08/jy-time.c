...
// JY Time watchface
// Samsung-style structure, emery target (Pebble Time 2, 200x228).
//
...
static GBitmap *s_w800_steps_bitmap;
...
static void draw_w800_steps_top_bar(GContext *ctx) {
  if (!s_w800_steps_bitmap || !s_w800_walking_bitmap) {
    return;
  }
  ...
  graphics_draw_bitmap_in_rect(ctx, s_w800_steps_bitmap, GRect(73, 3, 64, 24));
}
...
static void draw_complication(GContext *ctx, GPoint center, const char *value, int type) {
  ...
  draw_text(ctx, value, s_font_complication,
            GRect(center.x - 18, center.y + 3, 36, 16),
            GColorWhite, GTextAlignmentCenter);
}
...
static void face_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (s_w800_steps_top_enabled) {
    draw_complication(ctx, GPoint(40, 146), s_temp_buf, 1);
    draw_complication(ctx, GPoint(100, 146), s_rain_buf, 2);
    draw_complication(ctx, GPoint(160, 146), s_bpm_buf, 3);
  } else {
    draw_complication(ctx, GPoint(28, 146), s_steps_buf, 0);
    draw_complication(ctx, GPoint(76, 146), s_temp_buf, 1);
    draw_complication(ctx, GPoint(124, 146), s_rain_buf, 2);
    draw_complication(ctx, GPoint(172, 146), s_bpm_buf, 3);
  }

  graphics_context_set_stroke_color(ctx, GColorWhite);
  graphics_context_set_stroke_width(ctx, 1);
  graphics_draw_line(ctx, GPoint(8, 187), GPoint(192, 187));

  draw_text(ctx, s_event_buf, s_font_event, GRect(8, 198, 184, 24),
            GColorWhite, GTextAlignmentCenter);
}
...
static void inbox_received_handler(DictionaryIterator *iter, void *context) {
  ...
  if (t) {
    s_rain_chance = (uint8_t)t->value->uint8;
    s_rain_known = true;
...
static void window_load(Window *window) {
  ...
  s_font_complication = fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD);
  ...
  s_w800_steps_bitmap = gbitmap_create_with_resource(RESOURCE_ID_W800_STEPS_SPRITE);
...
static void window_unload(Window *window) {
  ...
  gbitmap_destroy(s_w800_steps_bitmap);
